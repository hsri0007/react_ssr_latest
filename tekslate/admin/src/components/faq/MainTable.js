...
function MainTable() {
  ...
  useEffect(() => {
    async function searchData() {
      const res = await fetch(
        `${url}/admin/faq/search?limit=${interval}&offset=${
          starting * interval
        }`,
        {
          method: "POST",
    ...
    async function fetchMyAPI() {
      const response = await fetch(
        `${url}/admin/faq?limit=${interval}&offset=${starting * interval}`,
        {
          headers: authHeader(),
        }
      );
    ...
    async function fetchDataCount() {
      const res = await fetch(`${url}/admin/faqCount`);
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const result = await fetch(`${url}/admin/faq/search`, {
        method: "POST",
        body: JSON.stringify({ query: search }),
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="main-table">
      <div className="upper">
        <h2 className="heading">View Faq</h2>
        <div>
          <Button
            variant="contained"
            color="secondary"
            ...
            startIcon={<AddIcon />}
            onClick={() => history.push("/faq/AddFaq")}
          >
            Add Faq
          </Button>
        </div>
                ...
                {fullData &&
                  Object.entries(fullData).map((item) => (
                    <StyledTableRow key={item[1].id}>
                      ...
                      <StyledTableCell align="center">
                        {htmlToText(item[1].faq_description, { wordwrap: 150 })}
                      </StyledTableCell>
                      <StyledTableCell align="center">
                        {item[1].f_status === 1 ? "Active" : "In Active"}
                      </StyledTableCell>
