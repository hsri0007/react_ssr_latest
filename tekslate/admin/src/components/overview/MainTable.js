...
function MainTable() {
  ...
  useEffect(() => {
    async function searchData() {
      const res = await fetch(
        `${url}/admin/overview/search?limit=${interval}&offset=${
          starting * interval
        }`,
        {
          method: "POST",
    ...
    async function fetchMyAPI() {
      const response = await fetch(
        `${url}/admin/overview?limit=${interval}&offset=${starting * interval}`,
        {
          headers: authHeader(),
        }
      );
    ...
    async function fetchDataCount() {
      const res = await fetch(`${url}/admin/overviewCount`);
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const result = await fetch(`${url}/admin/overview/search`, {
        method: "POST",
        body: JSON.stringify({ query: search }),
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="main-table">
      <div className="upper">
        <h2 className="heading">View Objective</h2>
        <div>
          <Button
            variant="contained"
            color="secondary"
            ...
            startIcon={<AddIcon />}
            onClick={() => history.push("/overview/AddOverview")}
          >
            Add Objective
          </Button>
        </div>
                ...
                {fullData &&
                  Object.entries(fullData).map((item) => (
                    <StyledTableRow key={item[1].id}>
                      ...
                      <StyledTableCell align="center">
                        {item[1].overview_title}
                      </StyledTableCell>
                      <StyledTableCell align="center">
                        {htmlToText(item[1].overview_description, {
                          wordwrap: 150,
                        })}
                      </StyledTableCell>
                      <StyledTableCell align="center">
                        {item[1].o_status === 1 ? "Active" : "In Active"}
                      </StyledTableCell>
                      <StyledTableCell align="center" className={item[1].id}>
                        <Button
                          ...
                          onClick={() =>
                            history.push({
                              pathname: "/overview/editOverview",
                              state: { id: item[1].id },
                            })
                          }
                        >
