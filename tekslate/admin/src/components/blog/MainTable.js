...
function MainTable() {
  ...
  useEffect(() => {
    async function searchData() {
      const res = await fetch(`${url}/admin/blog/search?limit=${interval}&offset=${(starting*interval)}`,{
        method: 'POST',
    ...
    async function fetchMyAPI() {
      const response = await fetch(`${url}/admin/blog?limit=${interval}&offset=${(starting*interval)}`, {
        headers: authHeader(),
      });
     ...
     async function fetchDataCount() {
      const res = await fetch(`${url}/admin/blogCount`)
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const result = await fetch(`${url}/admin/blog/search`, {
        method: "POST",
        body: JSON.stringify({ query: search }),
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="main-table">
      <div className="upper">
        <h2 className="heading">View Blog</h2>
        <div>
          <Button
            variant="contained"
            color="secondary"
            ...
            startIcon={<AddIcon />}
            onClick={() => history.push("/blog/addBlog")}
          >
            Add Blog
          </Button>
        </div>
