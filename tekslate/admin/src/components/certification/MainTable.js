...
function MainTable() {
  ...
  useEffect(() => {
    async function searchData() {
      const res = await fetch(
        `${url}/admin/certification/search?limit=${interval}&offset=${
          starting * interval
        }`,
        {
          method: "POST",
    ...
    async function fetchMyAPI() {
      const response = await fetch(
        `${url}/admin/certification?limit=${interval}&offset=${
          starting * interval
        }`,
        {
          headers: authHeader(),
    ...
    async function fetchDataCount() {
      const res = await fetch(`${url}/admin/certificationCount`);
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const result = await fetch(`${url}/admin/certification/search`, {
        method: "POST",
        body: JSON.stringify({ query: search }),
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="main-table">
      <div className="upper">
        <h2 className="heading">View Certification</h2>
        <div>
          <Button
            variant="contained"
            color="secondary"
            ...
            startIcon={<AddIcon />}
            onClick={() => history.push("/certification/AddCertification")}
          >
            Add Certification
          </Button>
        </div>
                ...
                {fullData &&
                  Object.entries(fullData).map((item) => (
                    <StyledTableRow key={item[1].id}>
                      ...
                      <StyledTableCell align="center">
                        {item[1].certification_title}
                      </StyledTableCell>
                      <StyledTableCell align="center">
                        {htmlToText(item[1].certification_description, {
                          wordwrap: 150,
                        })}
                      </StyledTableCell>
                      <StyledTableCell align="center">
                        {item[1].certification_status === 1
                          ? "Active"
                          : "In Active"}
                      </StyledTableCell>
                      <StyledTableCell align="center" className={item[1].id}>
                        <Button
                          ...
                          onClick={() =>
                            history.push({
                              pathname: "/certification/editCertification",
                              state: { id: item[1].id },
                            })
                          }
                        >
