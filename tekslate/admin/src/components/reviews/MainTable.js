...
function MainTable() {
  ...
  useEffect(() => {
    async function searchData() {
      const res = await fetch(
        `${url}/admin/review/search?limit=${interval}&offset=${
          starting * interval
        }`,
        {
          method: "POST",
    ...
    async function fetchMyAPI() {
      const response = await fetch(
        `${url}/admin/review?limit=${interval}&offset=${starting * interval}`,
        {
          headers: authHeader(),
        }
      );
    ...
    async function fetchDataCount() {
      const res = await fetch(`${url}/admin/reviewCount`);
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const result = await fetch(`${url}/admin/review/search`, {
        method: "POST",
        body: JSON.stringify({ query: search }),
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <div className="main-table">
      <div className="upper">
        <h2 className="heading">View Reviews</h2>
        <div>
          <Button
            variant="contained"
            color="secondary"
            ...
            startIcon={<AddIcon />}
            onClick={() => history.push("/reviews/addReviews")}
          >
            Add Reviews
          </Button>
        </div>
                ...
                {fullData &&
                  Object.entries(fullData).map((item) => (
                    <StyledTableRow key={item[1].id}>
                      ...
                      <StyledTableCell align="center">
                        {item[1].r_status === 1 ? "Active" : "In Active"}
                      </StyledTableCell>
