...
function MainTable() {
  ...
  useEffect(() => {
    async function searchData() {
      const res = await fetch(`${url}/admin/courses/search?limit=${interval}&offset=${(starting*interval)}`,{
        method: 'POST',
    ...
    async function fetchMyAPI(){
        const response = await fetch(`${url}/admin/courses?limit=${interval}&offset=${(starting*interval)}`, {
        headers: authHeader(),
      });
    ...
    async function fetchDataCount() {
      const res = await fetch(`${url}/admin/courseCount`)
      ...
      <div className="upper">
        ...
        <div>
          <Button
            ...
            startIcon={<AddIcon />}
            onClick={() => history.push("/courses/addCourse")}
          >
            Add Course
          </Button>
        </div>
